package com.moxi.hyblog.picture.restapi;


import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.moxi.hyblog.base.enums.EOpenStatus;
import com.moxi.hyblog.base.enums.EStatus;
import com.moxi.hyblog.base.validator.group.GetList;
import com.moxi.hyblog.base.vo.FileVO;
import com.moxi.hyblog.commons.feign.AdminFeignClient;
...
import com.moxi.hyblog.picture.entity.FileSort;
import com.moxi.hyblog.picture.global.SQLConf;
import com.moxi.hyblog.picture.global.SysConf;
import com.moxi.hyblog.picture.service.FileService;
import com.moxi.hyblog.picture.service.FileSortService;
import com.moxi.hyblog.picture.util.FeignUtil;
import com.moxi.hyblog.picture.util.QiniuUtil;
...
public class FileRestApi {
    ...
    private String path;
    @Value(value = "${file.reflect.path}" )
    private String reflectPath;

    @ApiOperation(value = "上传图片", notes = "上传图片", response = String.class)
    @PostMapping("/uploadPic")
    public String uploadPic(@RequestParam(value = "file")MultipartFile file) throws IOException {
        String url=System.currentTimeMillis()+"."+SysConf.JPG;
        file.transferTo(new File(path+url));
        return ResultUtil.result(SysConf.SUCCESS,reflectPath+url);
    }
    ...
    public String getPicture(
        ...
        if (StringUtils.isEmpty(fileIds)) {
            ...
            QueryWrapper<com.moxi.hyblog.picture.entity.File> queryWrapper = new QueryWrapper<>();
            queryWrapper.in(SQLConf.UID, changeStringToString);
            List<com.moxi.hyblog.picture.entity.File> fileList = fileService.list(queryWrapper);
            if (fileList.size() > 0) {
                for (com.moxi.hyblog.picture.entity.File file : fileList) {
                    if (file != null) {
    ...
    public Object downloadFile(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (StringUtils.isEmpty(fileId)) {
            ...
            com.moxi.hyblog.picture.entity.File oneById = fileService.getById(fileId);
            if (oneById != null) {
    ...
    public synchronized Object uploadPicsByUrl(HttpServletRequest request, @Validated({GetList.class}) @RequestBody FileVO fileVO, BindingResult result) {
        ...
        List<com.moxi.hyblog.picture.entity.File> lists = new ArrayList<>();
        ...
        if (urlList != null && urlList.size() > 0) {
            for (String itemUrl : urlList) {
                ...
                com.moxi.hyblog.picture.entity.File file = new com.moxi.hyblog.picture.entity.File();
