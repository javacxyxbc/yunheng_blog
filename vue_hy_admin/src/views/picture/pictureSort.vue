...
<div id="table" class="app-container calendar-list-container">
    ...
    <el-table :data="tableData"  style="width: 100%">
      ...
      <el-table-column label="序号" width="60" align="center">
	      ...
	      <template slot-scope="scope">
	      	<img  v-if="scope.row.CoverUrl" :src="scope.row.CoverUrl" style="width: 130px;height: 70px;"/>
	      </template>
	    </el-table-column>
	  ...
	  <!-- 添加或修改对话框 -->
		<el-dialog :title="title" :visible.sync="dialogFormVisible">
		  <el-form :model="form" :rules="rules" ref="form">
	    		...
	    		<div class="imgBody" v-if="form.url">
	    		  	<i class="el-icon-error inputClass" v-show="icon" @click="deletePhoto()" @mouseover="icon = true"></i>
	    			<img @mouseover="icon = true" @mouseout="icon = false" v-bind:src="form.url" style="display:inline; width: 195px;height: 105px;"/>
	    		</div>
		...
		<CheckPhoto v-if="photoVisible" @choose_data="getChooseData" @cancelModel="cancelModel" :photoVisible="photoVisible" :photos="photoList" :files="fileIds" :limit="1"></CheckPhoto>
...
export default {
  components: {
    CheckPhoto:resolve=>require(['../../components/CheckPhoto'],resolve)
  },
  created() {
    this.getDictList();
  ...
  methods: {
    ...
    getChooseData(data) {
      ...
      if (this.photoList.length >= 1) {
        ...
        this.form.url=this.photoList[0];
      }
    },
    //关闭模态框
    cancelModel() {
