...
import com.moxi.hyblog.xo.global.RedisConf;
import com.moxi.hyblog.xo.global.SQLConf;
import com.moxi.hyblog.xo.global.SysConf;
import com.moxi.hyblog.xo.service.*;
import com.moxi.hyblog.xo.utils.RabbitUtil;
import com.moxi.hyblog.xo.utils.RedisBitUtil;
...
import com.moxi.hyblog.xo.mapper.TagMapper;
...
public class BlogServiceImpl extends SuperServiceImpl<BlogMapper, Blog> implements BlogService {
    ...
    @Autowired
    private RabbitUtil rabbitMqUtil;
    ...
    public Blog getBlogById(String uid){
        QueryWrapper<Blog> queryWrapper=new QueryWrapper<>();
        queryWrapper.eq(SQLConf.TABLE_T1_UID,uid);
        Blog blog=blogMapper.getBlog(queryWrapper);
        blog.setUrl(reflectPath+blog.getUrl());
        blog.setUid(uid);
        return blog;
    }
    public Map getBlogByIdAsMap(String uid){
        QueryWrapper<Blog> queryWrapper=new QueryWrapper<>();
        queryWrapper.eq(SQLConf.TABLE_T1_UID,uid);
        Map blog=blogMapper.getBlogAsMap(queryWrapper);
        blog.put(SysConf.URL,reflectPath+blog.get(SysConf.URL));
        blog.put(SysConf.UID,uid);
        return blog;
    }
    public List<Blog> getBlogUids(){
        QueryWrapper<Blog> wrapper=new QueryWrapper<>();
        wrapper.eq(SQLConf.STATUS, EStatus.ENABLE);
        wrapper.eq(SQLConf.IS_PUBLISH, EPublish.PUBLISH);
        wrapper.select(SQLConf.UID,SQLConf.CLICK_COUNT,SQLConf.PRISE_COUNT);
        List<Blog> list=blogService.list(wrapper);
        return list;
    }
    public IPage<BlogSort> getRecommendSorts(){
        QueryWrapper<BlogSort> wrapper=new QueryWrapper<>();
        wrapper.eq(SQLConf.STATUS,EStatus.ENABLE);
        wrapper.select(SQLConf.UID,SQLConf.SORT_NAME,SQLConf.CLICK_COUNT);
        wrapper.orderByDesc(SQLConf.SORT);
        Page<BlogSort> page=new Page<>(0,20);
        return blogSortService.page(page,wrapper);
    }

    public IPage<Blog> getRecommendBlogList(String uid){
//         1.先搜索是否有相关文章
        //1.1到es查询
        ...
        wrapper.eq(SQLConf.STATUS,EStatus.ENABLE);
        wrapper.eq(SQLConf.IS_PUBLISH,EPublish.PUBLISH);
        wrapper.select(SQLConf.UID,SQLConf.TITLE,SQLConf.CLICK_COUNT);
        wrapper.orderByAsc(SQLConf.LEVEL);
        Page<Blog> page=new Page<>(0,20);
        return blogService.page(page,wrapper);
    }
    @Override
    ...
    public IPage<Blog> getPageList1(BlogVO blogVO){
        Page<Blog> page=new Page<>(blogVO.getCurrentPage(),blogVO.getPageSize());
        QueryWrapper<Blog> queryWrapper=new QueryWrapper<>();
        ...
        queryWrapper.eq(SQLConf.TABLE_T1_STATUS,EStatus.ENABLE);
        IPage<Blog> pageList=blogMapper.getBlogList(page,queryWrapper);
        for(Blog blog:pageList.getRecords()){
    ...
    //更改一
    //太麻烦,直接join连接就解决的事情,创建了一堆Map,list,
    @Override
    ...
    public String getBlogContent(String uid){
        ...
        wrapper.select(SQLConf.CONTENT);
        return ResultUtil.result(SysConf.SUCCESS,blogService.getOne(wrapper).getContent());
    }
    @Override
    public String addBlog(BlogVO blogVO) {
        ...
        if (EOriginal.ORIGINAL.equals(blogVO.getIsOriginal())) {
            //在请求中获取admin_uid,获取用户昵称
            QueryWrapper<Admin> query=new QueryWrapper();
            query.eq(SysConf.UID,request.getAttribute(SysConf.ADMIN_UID));
            query.select(SQLConf.NICK_NAME,SQLConf.UID);
        ...
        if(isSave){
            updateSolrAndRedis(isSave, blog,SysConf.ADD);
        }
    ...
    public String editBlog(BlogVO blogVO) {
        ...
        if (EOriginal.ORIGINAL.equals(blogVO.getIsOriginal())) {
            //在请求中获取admin_uid,获取用户昵称
            QueryWrapper<Admin> query=new QueryWrapper();
            query.eq("uid",request.getAttribute(SysConf.ADMIN_UID));
            query.select(SysConf.UID,SysConf.nickName);
            Admin admin=adminService.getOne(query);
            blog.setAuthor(admin.getNickName());
        } else {
            blog.setAuthor(blogVO.getAuthor());
        ...
        //读取redis中此时博客的点击量,访问量,并且发送消息到rabbitmq,表示删除这篇博客在redis中的缓存
        String key=SysConf.BLOG+SysConf.REDIS_SEGMENTATION+blogVO.getUid();
        if(redisUtil.hasKey(key)){
            ...
            rabbitMqUtil.sendDeleteBlogCommend(SysConf.DELETE,blogVO.getUid());
        }

        Boolean isSave = blog.updateById();
        if(isSave){
            //保存成功后，需要发送消息到solr 和 redis
            updateSolrAndRedis(isSave, blog,SysConf.EDIT);
        }
...
//        }
    ...
    public int updateCollectBatch(List<Blog> list) {
       return blogMapper.updateCollectBatch(list);
    }

    @Override
    public String deleteBlog(BlogVO blogVO) {
        int save =blogMapper.deleteBlog(blogVO.getUid());
    ...
    public String deleteBatchBlog(List<BlogVO> blogVoList) {
        ...
        StringBuilder deleteUids=new StringBuilder();
        blogVoList.forEach(item -> {
            list.add(item.getUid());
            deleteUids.append(item.getUid());
            deleteUids.append(SysConf.FILE_SEGMENTATION);
        });
        deleteUids.deleteCharAt(deleteUids.length()-1);
        int save=blogMapper.deleteBatchBlog(list);
        //保存成功后，需要发送消息到rabbitmq
        if (save>0) {
           rabbitMqUtil.sendDeleteBlogCommend(SysConf.DELETE_BATCH,deleteUids.toString());
        }
        return ResultUtil.result(SysConf.SUCCESS, MessageConf.DELETE_SUCCESS);
    }
    ...
    public IPage<Blog> getBlogPageByLevel(Integer level, Long currentPage, Integer useSort) {
        ...
        if ((level == SysConf.ONE || level == SysConf.TWO) && list.size() == 0) {
            ...
            for (int a = 0; a < hotBlogList.size(); a++) {
                ...
                if ((hotBlogList.size() - firstBlogList.size()) > 0) {
...
//        }
    ...
    public IPage<Blog> getBlogByTime(Long currentPage, Long pageSize) {
        ...
        IPage<Blog> pageList =blogMapper.getList(page,queryWrapper);
        for(Blog blog:pageList.getRecords()){
            blog.setUrl(reflectPath+blog.getUrl());
        }
        return pageList;
    ...
    public boolean priseBlog(String blogUid,String userUid){
        boolean flag=false;
        String key=SysConf.PRISE+SysConf.REDIS_SEGMENTATION+blogUid;
        int hashCode=userUid.hashCode();
        return bitUtil.thumbUp(key,hashCode);
    }
...
//        }
        ...
        queryWrapper.eq(SQLConf.BLOG_SORT_UID, blogSortUid);
        queryWrapper.orderByDesc(SQLConf.CREATE_TIME);
        queryWrapper.eq(SQLConf.TABLE_T1_PUBLIST, EPublish.PUBLISH);
        queryWrapper.eq(SQLConf.TABLE_T1_STATUS, EStatus.ENABLE);
        IPage<Blog> pageList = blogMapper.getList(page,queryWrapper);
        List<Blog> list = pageList.getRecords();
        for(Blog blog:pageList.getRecords()){
    ...
    public String getBlogTimeSortListY(){
        QueryWrapper<Blog> wrapper=new QueryWrapper<>();
        wrapper.eq(SQLConf.STATUS,EStatus.ENABLE);
        wrapper.eq(SQLConf.IS_PUBLISH,EPublish.PUBLISH);
        wrapper.select(SQLConf.UID,SQLConf.CREATE_TIME,SQLConf.TITLE);
        wrapper.orderByDesc(SQLConf.CREATE_TIME);
        List<Blog>  blogs=blogService.list(wrapper);
        Map<Integer,List<Blog>> blogTimeMap=new HashMap<>();
        for(Blog blog:blogs){
            int year=DateUtils.getYear(blog.getCreateTime());
            if(!blogTimeMap.containsKey(year)){
                blogTimeMap.put(year,new ArrayList<>());
            }
            blogTimeMap.get(year).add(blog);
        }
        List<archived> array=new ArrayList<>();
        for(int y:blogTimeMap.keySet()){
            archived a=new archived(y,blogTimeMap.get(y));
            array.add(a);
        }
        IPage<archived> page=new Page<>();
     ...
     * @param blog
     */
    private void updateSolrAndRedis(Boolean isSave, Blog blog,String type) {
        // 保存操作，并且文章已设置发布
        if (isSave && EPublish.PUBLISH.equals(blog.getIsPublish())) {
            ...
            //发送到RabbitMq
            rabbitTemplate.convertAndSend(SysConf.EXCHANGE_DIRECT, SysConf.HY_BLOG, map);

        } else if (EPublish.NO_PUBLISH.equals(blog.getIsPublish())) {

            //这是需要做的是，是删除redis和es中的该条博客数据
            Map<String, Object> map = new HashMap<>();
