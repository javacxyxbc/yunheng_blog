...
import com.moxi.hyblog.commons.entity.OnlineAdmin;
...
import com.moxi.hyblog.xo.mapper.AdminMapper;
import com.moxi.hyblog.xo.mapper.CategoryMenuMapper;
...
public class AdminServiceImpl extends SuperServiceImpl<AdminMapper, Admin> implements AdminService {
   ...
   //获取admin的权限列表，即url访问列表
    public String[] getAdminAuthorities(String uid){
        //现获取一个数组字符串[uid1,uid2,uid3]
        String authority=adminMapper.getAuthority(uid);
        String[] list=authority.replace("[", "").replace("]", "").replace("\"", "").split(",");
        ArrayList<String> categoryMenuUids=new ArrayList<>(Arrays.asList(list));
        return categoryMenuMapper.getAuthorities(categoryMenuUids);
    }
    @Override
